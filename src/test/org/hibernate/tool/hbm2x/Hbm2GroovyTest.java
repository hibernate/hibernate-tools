...
public class Hbm2GroovyTest extends NonReflectiveTestCase {

	private ArtifactCollector artifactCollector;
	
	public Hbm2GroovyTest(String name) {
		super( name, "hbm2groovyoutput" );
	}

	protected void setUp() throws Exception {
		super.setUp();

		Exporter exporter = new POGOExporter( getCfg(), getOutputDir() );
	...
	public void testFileExistence() {
		...
		assertFileAndExists( new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/Customer.groovy" ) );
		assertFileAndExists( new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/LineItem.groovy" ) );
		assertFileAndExists( new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/Order.groovy" ) );
		assertFileAndExists( new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/Train.groovy" ) );
		assertFileAndExists( new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/Passenger.groovy" ) );
		assertFileAndExists( new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/Product.groovy" ) );
		assertFileAndExists( new File( getOutputDir(),
				"generated/BaseHelloWorld.groovy" ) );
		assertFileAndExists( new File( getOutputDir(), "HelloUniverse.groovy" ) );
		
		assertFileAndExists( new File( getOutputDir(), "org/hibernate/tool/hbm2x/FatherComponent.groovy" ) );
		assertFileAndExists( new File( getOutputDir(), "org/hibernate/tool/hbm2x/ChildComponent.groovy" ) );

		assertEquals(15, artifactCollector.getFileCount("groovy"));
	}
...
//				.getAbsolutePath() );
//		TestHelper.compileGroovy( getOutputDir(), file, TestHelper.visitAllFiles(
//				getOutputDir(), list ) );
//
...
//	}
	...
	public void testNoFreeMarkerLeftOvers() {
		...
		assertEquals( null, findFirstString( "$", new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/Product.groovy" ) ) );
		assertEquals( null, findFirstString( "$", new File( getOutputDir(),
				"org/hibernate/tool/hbm2x/Address.groovy" ) ) );
	...
	public void testPackageName() {
		...
		Cfg2GroovyTool c2g = new Cfg2GroovyTool(new POGOHelper());;

		PersistentClass classMapping = getCfg()
				.getClassMapping( "org.hibernate.tool.hbm2x.Order" );
		POJOClass pc = c2g.getPOJOClass(classMapping);
		assertEquals(EntityPOGOClass.class, pc.getClass());
		...
		assertEquals( "did not honor generated-class", "package generated;", c2g.getPOJOClass( getCfg().getClassMapping(
	...
	public void testJavaDoc() {
		...
		assertEquals( " * test", c2g.toJavaDoc( "test", 0 ) );
		assertEquals( "   * test", c2g.toJavaDoc( "test", 2 ) );
		assertEquals( "   * test\n   * me", c2g.toJavaDoc( "test\nme", 2 ) );

		PersistentClass local = getCfg()
				.getClassMapping( "HelloWorld" );
		POJOClass pc = c2g.getPOJOClass(local);
	...
	public void testExtraCode() {
		...
		assertFalse( c2g.hasMetaAttribute( getCfg().getClassMapping(
		...
		assertEquals(
				...
				c2g.getPOJOClass(classMapping).getExtraClassCode() );
	}
	...
	public void testScope() {
		...
		PersistentClass pc = getCfg().getClassMapping(
				"org.hibernate.tool.hbm2x.Order" );
		assertEquals( "public strictfp", c2g.getClassModifiers( pc ) );

		assertEquals( "public", c2g.getClassModifiers( getCfg()
				.getClassMapping( "HelloWorld" ) ) );
	}
	...
	public void testDeclarationType() {
		...
		PersistentClass pc = getCfg().getClassMapping(
				"org.hibernate.tool.hbm2x.Order" );
		assertEquals( "class", c2g.getPOJOClass(pc).getDeclarationType() );

		assertEquals( "interface", c2g.getPOJOClass(getCfg()
	...
	public void testTypeName() {
		...
		assertEquals( "java.util.Collection", c2g.getJavaTypeName( property, false ) );
	...
	public void testUseRawTypeNullability() {
		...
		assertEquals( "typename should be used when rawtypemode", "int", c2g
				.getJavaTypeName( property, false ) );

		property = pc.getProperty( "minStock" );
		assertTrue( property.getValue().isNullable() );
		assertEquals( "typename should be used when rawtypemode", "long", c2g
				.getJavaTypeName( property, false ) );

		property = pc.getProperty( "otherStock" );
		assertFalse( property.getValue().isNullable() );
		assertEquals( "type should still be overriden by meta attribute",
				"java.lang.Integer", c2g.getJavaTypeName( property, false ) );

		property = pc.getIdentifierProperty();
		assertFalse( property.getValue().isNullable() );
		assertEquals( "wrappers should be used by default", "long", c2g
				.getJavaTypeName( property, false ) );
		...
		assertFalse( ((Property) identifier.getPropertyIterator().next() )
				.getValue().isNullable() );
		assertEquals( "long", c2g.getJavaTypeName( property, false ) );
	...
	public void testExtendsImplements() {
		...
		PersistentClass pc = getCfg().getClassMapping(
				"org.hibernate.tool.hbm2x.Order" );
		assertEquals( null, c2g.getPOJOClass(pc).getExtends() );

		POJOClass entityPOJOClass = c2g.getPOJOClass(getCfg().getClassMapping(
		"HelloWorld" ));
		...
		assertEquals( null, c2g.getPOJOClass(base).getExtends() );
		assertEquals( "Base", c2g.getPOJOClass(sub).getExtends() );
		...
		assertEquals( "Base,x,y", c2g.getPOJOClass(sub).getExtends() );
		...
		assertEquals( "intf,java.io.Serializable", c2g.getPOJOClass(base).getImplements() );
	}
	...
	public void testDeclarationName() {
		...
		POJOClass epc = c2g.getPOJOClass(pc);
		
		assertEquals( "Order", epc.getDeclarationName() );
		
		epc = c2g.getPOJOClass(hw);
	...
	public void testAsArguments() {
		...
		assertEquals(
				...
				c2g.asParameterList( pc.getPropertyIterator(), false, new NoopImportContext() ));
		assertEquals( "orderDate, total, customer, lineItems", c2g
	...
	public void testPropertiesForFullConstructor() {
		...
		POJOClass pjc = c2g.getPOJOClass(pc);
	...
	public void testEqualsHashCode() {
		...
		POJOClass pjc = c2g.getPOJOClass((Component) pc.getProperty("addressComponent").getValue());
